...
**v0.2.0 実装完了（ブランチ `feat/v0.2.0-foundations`、PR 待ち）。v0.1.0 は `Altena-Works/growth-loop` にタグ付きで保全済み（private）。**

### v0.2.0 で入れたもの

比較調査報告書の P0 のうち、基準値に依存しない3点。候補箱（残る P0）は、ナッジの実際の発火頻度が分からないうちに承認ステップを設計すると「無視される確認ダイアログ」になりうるため v0.3.0 へ回した。

**1. 判断区間全体を覆うロック。** 直したのは書き込みの原子性ではなく**読んで書く区間**である。cooldown の読み取りと状態の書き込みの間に `measure()` が挟まっていたため、同時に終了した2セッションが両方「直近の発火なし」を読み、両方発火していた。複数ターミナル・ワークツリーを開く運用では日常的に踏む。1本の advisory lock が読みから ledger 追記までを覆い、取れなかった側は「別セッションが判断中」として黙って終了する。状態書き込みは同一ディレクトリの一時ファイル＋`os.replace`。

実測: 修正前は20,000コールのトランスクリプトで10/10 レース再現、修正後は0/15。**保証は POSIX かつ書き込み可能なストレージ限定** — `fcntl` が無い環境やロックファイルを開けない場合は従来挙動に縮退する（フックがセッションを壊さないことを優先）。

**2. 操作契約（`contracts/operations.json`）。** プラグイン外、テストのみが読む。Claude Code が読むのは frontmatter であって我々の JSON ではなく、中に置けば14個目になるため。既に出荷して手で直した2種類の欠陥を機械検出可能にする: invoker のずれと、経路の受け入れ漏れ（`journey`→`refine` の契約矛盾。実セッションで発覚し、2巡のレビューが見逃した）。

初版は経路を4本落としていた。原因は計画が「完成品に見える JSON 例」を渡したことで、実装者はそれを検証したつもりで転記していた。散文を grep して全経路を突き合わせ直し、11経路・証拠クラス3種を追加した。

**3. 挙動ハーネス（`e2e/`、`GROWTH_LOOP_E2E=1` で opt-in）。** transcript の差分比較は実装しない。モデル出力は非決定的で、素直に diff すると欠陥と無関係に落ちる。代わりに**観測可能な帰結**を検査する: 実行後にどのファイルが存在するか、ゲートが保持されたか、スクリプトに到達したか、終了ステータス。7巡の欠陥修正で**ユニットテストがレビューに先んじて欠陥を見つけたことは一度も無く**、実害のあった欠陥は全て実行して判明した。ハーネスはその経路を初めて自動化する。

4ケース、いずれも v0.1.0 で実地検査が捕まえた欠陥クラスに対応。初回実行で自分自身の欠陥を1件見つけた（一覧を grep していたが `gl-journey` は28文字で名前を切るため、29文字の slug が「無い」と読めた）。`--locate` で解決する形に変更済み。

### 数字

- テスト **150/150 pass**、約2秒（`e2e/` は discover されない）
- ハーネス **4/4 pass**（`GROWTH_LOOP_E2E=1 python3 e2e/run_e2e.py`、1ケース1〜5分）
- プラグインは**ちょうど13ファイル**維持
- `claude plugin validate ./growth-loop` → Validation passed

**v0.1.0: 実装完了、実セッションで動作検証済み。**
